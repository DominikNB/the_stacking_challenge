...
lb = np.append(lb, 0)
ub = np.ones(np.sum(boxes[:, 4])) * 1
ub = np.append(ub, np.sum(boxes[:, 4]))

# Search for best stack
sh.stapler.opt = True
arguments = (boxes, items_vect, non_combine)
vars, fopt = pso(sh.stapler, lb, ub, args=arguments, debug=True, phip=10, phig=0.99, omega=0.1, minfunc=1e-64, minstep=1e-64, maxiter=30, swarmsize=5000)
